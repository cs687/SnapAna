function good=redseg_better_del_no_cells_auto_no_schmutz3_prime_v3(fname,handles, varargin)
...
currentfolder=[handles.current_directory,'\',handles.outputfolder];
if exist([currentfolder,'\Deleted Images'],'file')==0;
    mkdir([currentfolder,'\Deleted Images']);
end
...
for i = 1:Ncolors,
        D = dir([currentfolder,'\*-',colorcode(i),'.tif']);
    if length(D)>0, 
...
if useflat,
    ...
    for i = 1:length(ucolors),
        flatname = [handles.current_directory,'\',handles.outputfolder,'\','flat-',ucolors(i),'.tif'];
        if exist(flatname),
            flat = imread(flatname);
        else
            disp(['can''t find ',flatname,'...quitting']);
        end;
        flatbackname = [handles.current_directory,'\',handles.outputfolder,'\','flatback-',ucolors(i),'.tif'];
...
%removing +,' ',+,=,. from file name
    myname = [handles.current_directory,'\',handles.outputfolder,'\',fname];
    %mynamesimple = myname;
    mynamesimple = fname;
...
% Asigning value to Timestr, Gainstr, Exptime if they are empty
    ...
    %--------------------------------------------------------------------------------
                ...
                if cont==1
                    ...
                    for j=1:length(delcol)
                        ...
                        movefile([currentfolder,'\',file],delfolder);
                    end
                    %fclose(fileID);
                end;
            %else
    ...
    if ~isempty(S),
        eval(['save ',currentfolder,'\S',mynamesimple,suffix,' exptime S',mynamesimple,' Lq',mynamesimple,'*']);
    else
        eval(['save ',currentfolder,'\S',mynamesimple,suffix,' exptime ']);
    end;
...
clear S*;
D = dir([currentfolder,'\S*',suffix,'.mat']);
%if length(D)>0
    for i = 1:length(D), load([currentfolder,'\',D(i).name],'S*'); end;
%end
D_all=whos('S*');
if size(D_all,1)~=0
    save([currentfolder,'\sfile.mat'],'S*');
    good=1;
else
    good=0;
end
...
if min(size(imx))==1,
    ...
    if exist(imx)==2
        imx= imread([handles.current_directory,'\',handles.outputfolder,'\',imx]);
    end
end
if min(size(imx))>1
% get subimages
...
if size(r,1) == 1,
	...
	r = imread([handles.current_directory,'\',handles.outputfolder,'\',r]);
	g = imread([handles.current_directory,'\',handles.outputfolder,'\',g]);
	if nargin>=3,
		b = imread([handles.current_directory,'\',handles.outputfolder,'\',b]);
	end;
end;
